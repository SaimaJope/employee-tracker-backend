...
async function startServer() {
    ...
    app.post('/api/auth/register', async (req, res) => {
        ...
        try {
            ...
            if (error.code === 'SQLITE_CONSTRAINT') return res.status(409).json({ message: "A company or user with that name/email already exists." });
    ...
    app.get('/api/logs', authenticateToken, async (req, res) => {
        try {
            const logs = await db.all(`SELECT al.id, al.event_type, al.timestamp, al.nfc_card_id, e.name AS employee_name FROM attendance_logs AS al LEFT JOIN employees AS e ON al.employee_id = e.id WHERE al.company_id = ? ORDER BY al.timestamp DESC LIMIT 100`, req.user.companyId);
            res.json(logs);
        } catch (error) {
    ...
    app.post('/api/employees', authenticateToken, checkEmployeeLimit, async (req, res) => {
        try {
            ...
            res.status(500).json({ message: "Failed to add employee. Card ID might be in use." });
        }
    });
    ...
    app.put('/api/company/subscription', authenticateToken, async (req, res) => {
        try {
            const { plan } = req.body;
            if (!plan || !SUBSCRIPTION_PLANS[plan]) return res.status(400).json({ message: 'Invalid plan.' });
