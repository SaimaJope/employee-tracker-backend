// index.js - The final, complete, and robust backend code with all fixes
...
async function startServer() {
    ...
    // This helps with handling simultaneous requests better
    ...
    app.post('/api/employees', authenticateToken, async (req, res) => {
        ...
        try {
            ...
            if (!company) { await db.run('ROLLBACK'); return res.status(404).json({ message: "Company not found." }); }
            const countResult = await db.get('SELECT COUNT(*) AS count FROM employees WHERE company_id = ?', req.user.companyId);
            if (countResult.count >= company.max_employees) { await db.run('ROLLBACK'); return res.status(403).json({ message: `Employee limit of ${company.max_employees} reached.` }); }
    ...
    // Other routes for logs, kiosks, subscription updates etc.
    // ...
